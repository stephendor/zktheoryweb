...
def compute_persistence(distance_matrix: np.ndarray, max_dimension: int = 2) -> list:
...
def persistence_to_barcode(persistence: list, dimension: int) -> list[tuple[float, float]]:
