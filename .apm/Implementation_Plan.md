...
### Task 7.2 – TDA Paper Presentation Review & Schema Update - Agent_Content

**Objective:** Review and improve how TDA papers are presented across the site; replace the misleading `target_journal` field with properly gated `arxiv_id`, `arxiv_url`, `journal`, and `doi` fields that are only populated when information becomes available.
**Output:** Updated Zod schema; updated PaperLayout hero; all 10 paper MDX files corrected; TDA hub paper cards updated; no paper shows a journal name unless it is actually accepted/published.
**Guidance:** Currently `target_journal` is rendered prominently in the hero on every paper page, listing specific journals (e.g. "Sociological Methodology") for papers still in progress. This is misleading. BibTeX blocks inside `bibtex:` fields also carry journal names for unpublished papers. The fix is to gate all publication metadata behind optional fields. **Depends on: Phase 6 complete**

1. **Schema update** (`src/content.config.ts`): Remove `target_journal`. Add four new optional fields to the `papers` collection:
   - `arxiv_id: z.string().optional()` — arXiv identifier e.g. `"2501.12345"`
   - `arxiv_url: z.string().optional()` — full arXiv abstract URL
   - `journal: z.string().optional()` — journal name, populated only when accepted/published
   - `doi: z.string().optional()` — DOI, populated only when published
2. **PaperLayout hero** (`src/layouts/PaperLayout.astro`): Replace the `{target_journal && <p>}` block with conditional badges: show an arXiv badge + link if `arxiv_id` is present; show a journal badge if `journal` is present; show nothing if neither is present. Destructure new fields from `entry.data`.
3. **All 10 paper MDX files** (`src/content/tda/papers/paper-01.mdx` through `paper-10.mdx`): Remove all `target_journal:` frontmatter lines. Clean up `bibtex:` entries — for any paper without a journal acceptance, remove the `journal = {…}` and `doi = {}` BibTeX lines (or replace with a `note = {Forthcoming}` pattern). Set paper statuses accurately: Paper 1 is `in-progress`, Papers 2–10 are `planned`. Do not add `arxiv_id`/`arxiv_url` yet — those fields will be populated when submissions occur.
4. **TDA hub** (`src/pages/tda/index.astro`) and paper cards: Audit any rendering of `target_journal` on the hub or card components; remove or replace with the new field structure.
5. **Build verification**: `npm run build` (confirm 127 pages), `npm test` (confirm 420 tests), lint 0 errors. Commit and push to `main`.

### Task 7.7 – Orshansky Essay: LaTeX to MDX Conversion - Agent_Content

**Objective:** Convert the BSHM 2025 prize essay `content-source/writing/Orshansky.tex` to a production MDX file in the writing essays collection, mark demo stubs as drafts, and add the essay to the site.
**Output:** `src/content/writing/essays/orshansky-poverty-line.mdx` with fully converted content; `sample-essay.mdx` and `sample-note.mdx` marked `draft: true`; image placeholder in place ready for manual image supply.
**Guidance:** Do not fabricate or alter the essay's argument or prose — convert faithfully from LaTeX. Do not attempt to source or generate the figure image; include an `<img>` tag with the correct path and a clear code comment that the file must be manually placed. **Depends on: Task 7.6 complete**

1. **Mark demo stubs as draft**: In `src/content/writing/essays/sample-essay.mdx` and `src/content/writing/notes/sample-note.mdx`, change `draft: false` to `draft: true`.
2. **Create** `src/content/writing/essays/orshansky-poverty-line.mdx` with frontmatter: title, date `2025-06-24`, tags `['poverty measurement', 'history of mathematics', 'statistics', 'social justice']`, `draft: false`, summary (~50 words), description (~30 words).
3. **Convert LaTeX body to MDX**: `\section{}` → `##`; `\textit{}` → `*italics*`; `\textbf{}` → `**bold**`; `\begin{displayquote}` → `>`; `\begin{equation}` → `$$...$$`; LaTeX `--` → `—`; `~` → space; `\url{}` → markdown link. Leave in-text citations as plain text.
4. **Figure**: Replace `\begin{figure}` block with an `<img>` tag referencing `/images/orshansky-table1.png` with a descriptive `alt` attribute and a `<figcaption>`. Add a code comment that the image file must be manually placed at `public/images/orshansky-table1.png`.
5. **Prize note**: Add `*This essay won the British Society for the History of Mathematics Undergraduate Essay Prize, 2025.*` immediately after the opening prose paragraph, before the first `##` section heading.
6. **Bibliography**: Convert to `## References` section; each `\bibitem` as a formatted paragraph; `*Italics*` for titles; markdown links for URLs.
7. **Verify and commit**: `npm test`, `npm run lint`, `npm run build`. Commit: `content: add Orshansky BSHM essay (LaTeX to MDX conversion)`. Push to `main`. Note in Memory Log that `public/images/orshansky-table1.png` must be manually supplied.

---

## Phase 8: Advanced Content Creation

### Task 8.1 – Unlock Learning Paths 3 & 4 — DEFERRED

**Status: On hold pending author review of path content before publish.**
Paths 3 (Data Justice) and 4 (TDA for Practitioners) are `available: false` in `src/pages/learn/index.astro`. All MDX module stubs exist (path3: 6 modules, path4: 14 modules). No content changes needed — unlock requires flipping two `available` flags. Issue this task when ready.

---

### Task 8.2 – Thread Pages: Dynamic Route and Layout - Agent_Design_Templates

**Objective:** Build the `/counting-lives/threads/[slug]/` route so that thread indicator pills on chapter pages resolve to real pages, and render the authored Scottish Thread and Gender Thread MDX content.
**Output:** `src/pages/counting-lives/threads/[slug].astro` dynamic route; thread pages live at `/counting-lives/threads/scottish-thread/` and `/counting-lives/threads/gender-thread/`. **Depends on: Phase 7 complete**

**Context the agent must know before starting:**
- The `threads` collection is defined in `src/content.config.ts`. Schema: `title: z.string()`, `thread_id: z.enum(['scottish', 'gender'])`, `description: z.string()`, `status: z.enum(['drafting','in-review','complete']).default('drafting')`.
- Content files: `src/content/counting-lives/threads/scottish-thread.mdx` and `gender-thread.mdx`. Both are rich authored MDX with Introduction, Key Figures Timeline, Chapter Annotations, and a reading list. The status frontmatter of both is `drafting`.
- `ThreadMarker.astro` already links pills to `/counting-lives/threads/{slug}/`. Color mapping: `scottish-thread` → `var(--color-cl-red)`, `gender-thread` → `var(--color-cl-ochre)`.
- Use `src/pages/counting-lives/chapters/[slug].astro` as the structural reference for how to build a `getStaticPaths()` route over a content collection.
- Style using the Counting Lives "Archival" palette (`--color-cl-cream`, `--color-cl-red`, `--color-cl-ochre`, `--color-cl-charcoal`). Use `--color-cl-ochre` as the hero accent for the gender thread and `--color-cl-red` for the Scottish thread — derive the accent from `thread_id`.
- Do **not** create a new layout file — inline the layout within `[slug].astro` using `BaseLayout.astro` as the outer shell, following the same pattern as other simple pages (e.g. `src/pages/counting-lives/bibliography.astro`).

**Steps:**

1. **Create `src/pages/counting-lives/threads/[slug].astro`**:
   - `getStaticPaths()`: `getCollection('threads')`, map each entry to `{ params: { slug: entry.id }, props: { entry } }`.
   - Render with `const { Content, headings } = await render(entry)`.
   - Hero band (same structure as chapter hero): part label "Counting Lives — Thematic Thread", thread title as `<h1>`, description as subtitle. Hero background `var(--color-cl-cream)`, bottom border `var(--color-neutral-border)`. Accent colour from `thread_id`: `'scottish' → var(--color-cl-red)`, `'gender' → var(--color-cl-ochre)`. Apply accent as a left border stripe on the hero or as the `hero-chapter-number` equivalent label colour.
   - Status badge: same `status-badge status-{status}` pattern as `ChapterLayout.astro`.
   - Body: `.container-prose--wide` with `.prose` article containing `<Content />`, and a `<StickyToC>` sidebar using `section="counting-lives"`.
   - No prev/next navigation needed — there are only two threads.
   - `<BaseLayout title={title} description={description} section="counting-lives" />`

2. **Chapter backreference panel**: Below the prose, render a `<ConnectionsPanel>` showing all chapters that carry this thread's slug. Query: `await getCollection('chapters', ({ id }) => id !== 'ch-00-sample')`, filter where `entry.data.threads.includes(entry.id)` — note: `entry.id` at the outer scope is the thread slug (e.g. `'scottish-thread'`); use a variable name to avoid shadowing. Map matching chapters to `{ href: '/counting-lives/chapters/{ch.id}/', label: 'Chapter {ch.data.chapter_number}', title: ch.data.title, palette: 'cl' }`. Pass as `connections` (flat mode) to `ConnectionsPanel` with `heading="Chapters in This Thread"`.

3. **Update `src/pages/counting-lives/index.astro`**: Add a "Thematic Threads" section linking to `/counting-lives/threads/scottish-thread/` and `/counting-lives/threads/gender-thread/`, consistent in style with existing index sections. Position it after the chapter list and before the bibliography link.

4. **Verify and commit**:
   - `npm test` — passes
   - `npm run build` — passes, page count increases by 2 (one per thread)
   - Visit `/counting-lives/threads/scottish-thread/` and `/counting-lives/threads/gender-thread/` in the preview and confirm pill links from Ch.1 and Ch.2 resolve correctly
   - Commit: `feat: add thread pages route for Scottish and Gender threads`
   - Push to `main`

---

### Task 8.3 – Glossary Expansion to ~50 Terms - Agent_Content

**Objective:** Expand `src/data/glossary.ts` from 26 entries to approximately 50 by adding ~24 new `GlossaryEntry` objects — covering missing TDA terms and missing Counting Lives / poverty measurement terms — drawn from the existing site content.
**Output:** `src/data/glossary.ts` with ~50 entries; no schema changes; build and tests pass. **Depends on: Task 8.2 complete** (so thread pages exist and new `linkedContent` can reference them if needed)

**Context the agent must know before starting:**
- Every entry must conform to the existing `GlossaryEntry` interface exactly: `term`, `slug` (kebab-case, globally unique), `definitions` (1–2 items, each `{ domain: 'TDA' | 'Counting Lives' | 'Shared', text: string }`), `relatedTerms` (slugs of other entries), `linkedContent: { chapters: number[], papers: number[], methods: string[], modules: string[] }`.
- New entries must be appended before the closing `];` of `glossaryEntries`, after the existing entry for `'Normalisation'`.
- All definitions must be drawn faithfully from the existing MDX content in `src/content/counting-lives/chapters/`, `src/content/tda/papers/`, and the thread MDX files. Do not invent definitions. Read the relevant source files before writing each entry.
- The `linkedContent.chapters` array uses chapter numbers (integers), not slugs. `linkedContent.papers` uses paper numbers.
- `relatedTerms` must only reference slugs that already exist OR slugs of other new entries being added in this same task.

**New TDA terms to add** (8 terms — read TDA paper MDX files for definitions):

| Term | Slug | Source |
|------|------|--------|
| Barcode | `barcode` | Paper 1 / TDA path modules |
| Bottleneck Distance | `bottleneck-distance` | Paper 1 |
| Birth–Death Pair | `birth-death-pair` | Paper 1, learn modules |
| Čech Complex | `cech-complex` | Paper 2 |
| Persistence Landscape | `persistence-landscape` | Paper 3 |
| Nerve Theorem | `nerve-theorem` | Paper 2 |
| Interleaving Distance | `interleaving-distance` | Paper 3 |
| Zigzag Persistence | `zigzag-persistence` | Paper 3 |

**New Counting Lives / Shared terms to add** (~16 terms — read chapter MDX files for definitions):

| Term | Slug | Primary source |
|------|------|----------------|
| Beveridge Report | `beveridge-report` | Ch.3 |
| Male Breadwinner Model | `male-breadwinner-model` | Ch.3, Ch.4, gender-thread.mdx |
| Subsistence Standard | `subsistence-standard` | Ch.1 |
| Rowntree Survey | `rowntree-survey` | Ch.1, Ch.13 |
| Statistical Account of Scotland | `statistical-account-of-scotland` | scottish-thread.mdx |
| Regression to the Mean | `regression-to-the-mean` | Ch.2, scottish-thread.mdx |
| Contributory Insurance | `contributory-insurance` | Ch.3 |
| FGT Index | `fgt-index` | Ch.6 or CL content — if not found, derive from the Foster–Greer–Thorbecke poverty measures concept |
| Gini Coefficient | `gini-coefficient` | Ch.6 / shared statistical knowledge |
| Multidimensional Poverty Index | `multidimensional-poverty-index` | Ch.15 or data justice content |
| Capabilities Approach | `capabilities-approach` | Ch.17 / Amartya Sen |
| Means Testing | `means-testing` | Ch.13 |
| Less Eligibility | `less-eligibility` | Ch.13 |
| Participatory Statistics | `participatory-statistics` | Ch.15, `counter-mathematics` glossary entry |
| New Jim Code | `new-jim-code` | Ch.10, Ch.11 |
| L'homme moyen | already exists — skip | — |

Note: if a term's definition cannot be found in any site content file, write a concise 1–2 sentence definition consistent with the book's intellectual framework and mark `domain: 'Shared'`. Do not skip terms — provide all ~24.

**Steps:**

1. Read `src/data/glossary.ts` in full to understand the existing 26 entries and avoid slug collisions.
2. Read the source MDX files listed above for each new term group. Read TDA paper MDX files for TDA terms; read chapter MDX files and thread MDX files for CL terms.
3. Append all new entries to `glossaryEntries` (before the closing `];`), formatted consistently with existing entries — same indentation, same comment style (TDA and CL blocks separated by `// ── TDA Terms ──` and `// ── Counting Lives / Poverty Measurement Terms ──` sub-comments).
4. Run `npm test` — passes.
5. Run `npm run build` — passes.
6. Commit: `content: expand glossary from 26 to ~50 terms`.
7. Push to `main`.
...
### Task 7.6 – Homepage Latest Updates — Mixed Editorial Feed - Agent_Schema_Platform

**Objective:** Replace the broken "sort by number descending" Latest Updates feed on the homepage with a three-slot mixed feed: one featured chapter (editorial flag), one featured paper (editorial flag), and the most recent non-draft writing entry (essays or notes, sorted by date desc).
**Output:** Updated Zod schemas for chapters and papers (add `featured` boolean); updated `src/pages/index.astro` feed logic; `featured: true` set on ch-01.mdx and paper-01.mdx; section gracefully handles zero writing entries. **Depends on: Task 7.1 complete**

1. **Schema updates** (`src/content.config.ts`): Add `featured: z.boolean().default(false)` to both the `chapters` collection schema and the `papers` collection schema.
2. **Frontmatter updates**: Set `featured: true` in `src/content/counting-lives/chapters/ch-01.mdx` and `src/content/tda/papers/paper-01.mdx`. No other chapter or paper files need touching.
3. **Homepage feed logic** (`src/pages/index.astro`): Replace the current `recentChapters` / `recentPapers` sort-by-number queries with:
   - `featuredChapter`: first chapter where `data.featured === true` and `id !== 'ch-00-sample'`
   - `featuredPaper`: first paper where `data.featured === true`
   - `latestWriting`: query both `essays` and `notes` collections, filter out `draft: true` entries, merge, sort by `data.date` descending, take the first result (may be undefined — handle gracefully)
4. **Template update** (`src/pages/index.astro`): Rewrite the `activity-list` render block to use the three slots. If `latestWriting` is undefined (no published writing entries yet), render only the two editorial slots with no gap or error. Badge labels: "Chapter", "Paper", "Essay" or "Note" (derived from which collection the writing entry came from). Writing entries link to their writing page route.
5. **Rename section heading** from "Latest Updates" to "Selected Work & Writing" to accurately describe the mixed editorial/feed nature.
6. **Verification**: `npm test` (all tests pass), `npm run lint` (0 errors). Confirm the three slots render correctly with `npx serve dist` after `npm run build`. Commit: `feat: homepage mixed editorial feed (featured chapter/paper + latest writing)`. Push to `main`.
